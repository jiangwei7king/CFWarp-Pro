...
wget -N -4 https://cdn.jsdelivr.net/gh/YG-tsj/Oracle-warp/wgcf
...
apt update
apt -y --no-install-recommends install openresolv dnsutils wireguard-tools
wget -N -4 https://cdn.jsdelivr.net/gh/Oracle-warp/wgcf
cp wgcf /usr/local/bin/wgcf
chmod +x /usr/local/bin/wgcf
echo | wgcf register
wgcf generate
...
function start_menu(){
    ...
    yellow " 请选择（共1~17选项）！！！进入脚本快捷方式bash ~/multiARM.sh （如脚本更新，请先执行完整脚本）"
