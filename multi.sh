...
rv4=`ip addr | grep -E 'ens|eth0|enp0s3' | cut -d'/' -f1 | awk 'NR==2 {print $2}'`
...
function start_menu(){
    ...
    green " 1. 开启甲骨文VPS的ubuntu系统所有端口。自动断连后，请重新连接SSH（甲骨文云用户建议选择！！） "
...
function start_menu(){
    ...
    green " 2. 更新ARM架构通用版内核至5.11版。自动断连后，请重新连接SSH "
